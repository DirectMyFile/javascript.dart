...
Program parse(String text, {String filename, int firstLine : 1, bool handleNoise : true, bool annotations : true}) {
  Offsets offset = new Offsets(0, text.length, firstLine);
  if (handleNoise) {
    offset = trimNoise(text, offset);
  }
  Lexer lexer = new Lexer(text, filename: filename, currentLine: offset.line, index : offset.start, endOfFile : offset.end);
